...
#模型訓練
#gbm = lgb.LGBMRegressor(num_leaves=25, learning_rate=0.1, n_estimators=30, max_depth= 25)
#gbm.fit(np.array(Xtrain), np.array(Ytrain))

# 預測
#y_predicted = gbm.predict(np.array(Xtest))
...
num_var = Train[['P_CNT', 'school_活動集結點', 'ACT_score', 'Tran_score', 'PASSNUM']]
...
###StopSequence_START to StopSequence_Start
...
df[(Test[Weekday_name]== 1)].to_csv('205_0_PRED_2.csv', index = False, encoding='ansi')


####
Train_2 = Train[(Train['Byyyymmdd']== 20191018)|(Train['Byyyymmdd']== 20191025)|(Train['Byyyymmdd']== 20191101)|(Train['Byyyymmdd']== 20191108)]
Train_2.Byyyymmdd.unique()
X_test_2 = Train_2[['RouteName_Zh_tw', 'StopName_START', 'StopSequence_START', 'month', 'Direction', 'TripID', 'big_holiday', 'typhoon', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday', '是否下雨', 'rain_level', 'Break', 'P_CNT', 'school_活動集結點', 'ACT_score', 'Tran_score']]
print(X_test_2.columns[2:])
test_col = X_test_2.columns[2:]
Xtest = X_test_2[test_col].values
y_predicted = model_2.predict(np.array(Xtest))
y_predicted= [ round(i)  for i in  y_predicted.tolist() ]
passnum_predict = y_predicted
Train_2['passnum_predict'] = np.abs(y_predicted)
Train_2['gap'] = np.abs(Train_2['passnum_predict'] - Train_2['PASSNUM'])

plt.figure(figsize = (50, 20))
sns.pointplot(x = 'StopName_START', y = 'passnum_predict', hue = 'Byyyymmdd', data = Train_2[(Train_2['TripID']==10)] )
plt.figure(figsize = (50, 20))
sns.pointplot(x = 'StopName_START', y = 'PASSNUM', hue = 'Byyyymmdd', data = Train_2[(Train_2['TripID']==10)] )
plt.show()

###Graph
dftest['passnum_predict'] = Testing['predict_passnum']
plt.figure(figsize = (50, 20))
#df2 = dftest[(dftest['Byyyymmdd']== 20190906)][(dftest[(dftest['Byyyymmdd']== 20190906)]['TripID']== 13)]
df2 = dftest[(dftest['Byyyymmdd']== 20191108)][(dftest[(dftest['Byyyymmdd']== 20191108)]['TripID']== 12)]
sns.pointplot(x = 'StopName_START', y = 'passnum_predict', color = 'green', data = df2)
sns.pointplot(x = 'StopName_START', y = 'PASSNUM', data = df2)
plt.legend(['predict', 'passnum'])
plt.show()

sns.countplot(x='Makert', data = Train[(Train['TripID']==12)])
